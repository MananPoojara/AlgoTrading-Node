...
const { BaseStrategy, STRATEGY_STATES } = require('../../../packages/strategies/baseStrategy');
...
class StrategyWorker {
  ...
  async initialize() {
    ...
    await this.subscribeToExecutionUpdates();
    this.startHeartbeat();
  ...
  async subscribeToExecutionUpdates() {
    ...
    try {
      if (typeof subscriber.subscribeToOrderUpdatesStream !== "function") {
        throw new Error("order update stream subscription unavailable");
      }

      await subscriber.subscribeToOrderUpdatesStream((message) => {
        this.handleExecutionUpdate(message);
      });
    } catch (error) {
      logger.warn("Order updates stream subscription failed — falling back to Pub/Sub", {
        error: error.message,
      });
      await subscriber.subscribe("order_updates", (message) => {
        this.handleExecutionUpdate(message);
      });
    }

    try {
      if (typeof subscriber.subscribeToRejectedOrdersStream !== "function") {
        throw new Error("rejected order stream subscription unavailable");
      }

      await subscriber.subscribeToRejectedOrdersStream((message) => {
        this.handleExecutionUpdate(message);
      });
    } catch (error) {
      logger.warn("Rejected orders stream subscription failed — falling back to Pub/Sub", {
        error: error.message,
      });
      await subscriber.subscribe("rejected_orders", (message) => {
        this.handleExecutionUpdate(message);
      });
    }
  ...
  handleExecutionUpdate(update) {
    const instanceId = update?.strategy_instance_id;
    if (!instanceId) {
      return;
    }

    const strategy = this.strategies.get(instanceId);
    if (!strategy || typeof strategy.handleExecutionUpdate !== "function") {
      return;
    }

    strategy.handleExecutionUpdate(update);
  }

  async restoreState(instanceId, runtimeState = {}) {
    const strategy = this.strategies.get(instanceId);
    if (!strategy) {
      logger.warn("Cannot restore strategy state - strategy not registered", {
        workerId: this.id,
        instanceId,
      });
      return false;
    ...
    strategy.runtimeState = nextRuntimeState;

    const restoreStrategyState =
      typeof strategy.restoreState === "function"
        ? strategy.restoreState.bind(strategy)
        : typeof strategy.restoreRuntimeState === "function"
          ? strategy.restoreRuntimeState.bind(strategy)
          : null;

    if (restoreStrategyState) {
      await restoreStrategyState(nextRuntimeState);
    }
    ...
    logger.info("Strategy state restored", {
      ...
      restoredByHook: Boolean(restoreStrategyState),
    });
  ...
  async squareOff(reason = 'scheduler_square_off') {
    ...
    for (const [_, strategy] of this.strategies) {
      if (typeof strategy.squareOff === 'function') {
        await strategy.squareOff(reason);
        affectedAny = true;
      }
    }
...
class WorkerManager {
  ...
  async subscribeToControlCommands() {
    ...
    await subscriber.subscribeToStrategyControl(async (message) => {
      await this.handleStrategyControl(message);
    });
  }
  ...
  async handleStrategyControl(message = {}) {
    const command = String(message.command || '').toLowerCase();

    switch (command) {
      case 'pause_all_strategies':
        this.pauseAllStrategies();
        break;
      case 'resume_all_strategies':
        this.resumeAllStrategies();
        break;
      case 'square_off_all_strategies':
        await this.squareOffAllStrategies(message.reason || 'scheduler_square_off');
        break;
      default:
        logger.warn('Invalid strategy control command received', { command });
        break;
    }
  }
  ...
  async startStrategy(instanceId, config) {
    ...
    const recoveredRuntimeState = await loadStrategyRecoveryState(
      instanceId,
      runtimeState,
    );
    ...
    const strategy = new StrategyClass({
      ...
      runtimeState: recoveredRuntimeState,
      persistRuntimeState: async (_, nextRuntimeState) =>
        persistStrategyRuntimeState(instanceId, nextRuntimeState),
    });

    worker.registerStrategy(instanceId, strategy);
    await worker.restoreState(instanceId, recoveredRuntimeState);
  ...
  async squareOffAllStrategies(reason = 'scheduler_square_off') {
    for (const worker of this.workers.values()) {
      await worker.squareOff(reason);
    }
  }
    ...
    for (const mapping of mappings) {
      try {
        await this.startStrategy(mapping.instanceId, {
          ...
          runtimeState: mapping.runtimeState || {},

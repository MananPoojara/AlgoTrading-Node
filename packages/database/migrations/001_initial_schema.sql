...
CREATE INDEX IF NOT EXISTS idx_accounts_broker_name ON accounts(broker_name);
...
CREATE INDEX IF NOT EXISTS idx_strategy_instances_client_id ON strategy_instances(client_id);
CREATE INDEX IF NOT EXISTS idx_strategy_instances_strategy_id ON strategy_instances(strategy_id);
CREATE INDEX IF NOT EXISTS idx_strategy_instances_status ON strategy_instances(status);
CREATE INDEX IF NOT EXISTS idx_strategy_instances_worker_id ON strategy_instances(worker_id);
...
CREATE INDEX IF NOT EXISTS idx_signals_strategy_instance_id ON signals(strategy_instance_id);
...
CREATE INDEX IF NOT EXISTS idx_orders_strategy_instance_id ON orders(strategy_instance_id);
...
CREATE INDEX IF NOT EXISTS idx_positions_strategy_instance_id ON positions(strategy_instance_id);
...
CREATE INDEX IF NOT EXISTS idx_portfolio_snapshots_client_id ON portfolio_snapshots(client_id);
CREATE INDEX IF NOT EXISTS idx_portfolio_snapshots_snapshot_time ON portfolio_snapshots(snapshot_time DESC);
...
CREATE INDEX IF NOT EXISTS idx_operator_audit_log_operator_id ON operator_audit_log(operator_id);
CREATE INDEX IF NOT EXISTS idx_operator_audit_log_action ON operator_audit_log(action);
CREATE INDEX IF NOT EXISTS idx_operator_audit_log_timestamp ON operator_audit_log(timestamp DESC);
...
-- Apply updated_at triggers (idempotent — only creates if trigger does not exist)
DO $$
DECLARE
  t RECORD;
BEGIN
  FOR t IN
    SELECT tbl, trig FROM (VALUES
      ('clients',            'update_clients_updated_at'),
      ('accounts',           'update_accounts_updated_at'),
      ('strategies',         'update_strategies_updated_at'),
      ('strategy_instances', 'update_strategy_instances_updated_at'),
      ('orders',             'update_orders_updated_at'),
      ('operators',          'update_operators_updated_at')
    ) AS v(tbl, trig)
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM pg_trigger WHERE tgname = t.trig
    ) THEN
      EXECUTE format(

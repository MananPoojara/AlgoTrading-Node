...
function evaluateStrategy1(bars, state = {}, options = {}) {
  ...
  if (!state.inPosition) {
    ...
    if (state.lastEvaluatedDate === latestBar.date) {
    ...
    return {
      ...
      indicators,
      entryDate: latestBar.date,
    };
  }

  const entryIndex = bars.findIndex((bar) => bar.date === state.entryDate);
  ...
  for (let index = entryIndex + 1; index < bars.length; index += 1) {
    ...
    if (Number(bars[index].close) < trailingStop) {
      if (index === latestIndex && state.lastEvaluatedDate !== latestBar.date) {
        return {
          action: "SELL",
          reason: "atr_trailing_exit",
          referencePrice: round2(latestBar.close),
          indicators,
          exitDate: latestBar.date,

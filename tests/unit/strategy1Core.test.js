const { evaluateStrategy1 } = require("../../packages/strategies/intraday/strategy1Core");

describe("strategy1Core", () => {
  it("emits a BUY decision when fixed maxRed 3 is reached", () => {
    const bars = [
    ...
    const result = evaluateStrategy1(
      bars,
      { inPosition: false, lastEvaluatedDate: null, symbol: "NIFTY 50" },
      { ticker: "NIFTY 50", fixedMax: true, maxRed: 3 },
    );

    expect(result.action).toBe("BUY");
    expect(result.reason).toBe("consecutive_red_entry");
    expect(result.entryDate).toBe("2026-03-03");
    expect(result.referencePrice).toBe(97);
  });
